...
if [[ ${PRE_TRAIN_STANCE} = true ]]; then
    echo "Training pre stance model..."
    python stance/stance_train.py \
      ...
      --coaid_sentiment_path ${DATASET_PATH}/coaid_tweets_sentiment.json \
      --coaid_emotion_path ${DATASET_PATH}/coaid_tweets_emotion.json \
      --coaid_irony_path ${DATASET_PATH}/coaid_tweets_irony.json \
      --split_path ${DATASET_PATH}/split_1.json \
      --pre_model_name ${STANCE_PRE_MODEL_NAME} \
      --model_name stance-${DATASET}-${RUN_NAME}_PRE_${RUN_ID} \
...
for (( SPLIT=1; SPLIT<=${NUM_STANCE_SPLITS}; SPLIT++ )) do
    if [[ ${TRAIN_STANCE} = true ]]; then
        ...
        python stance/stance_train.py \
          ...
          --load_checkpoint models/stance-${DATASET}-${RUN_NAME}_PRE_${RUN_ID}/pytorch_model.bin \
          --max_seq_len ${STANCE_MAX_SEQ_LEN} \
          --batch_size ${STANCE_BATCH_SIZE} \
          --learning_rate 5e-5 \
          --epochs 20 \
